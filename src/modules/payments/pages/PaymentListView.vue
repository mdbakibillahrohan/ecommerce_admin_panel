...
import { paymentsApi, type Payment } from '@/modules/payments/api/payments'
import {
  ReloadOutlined,
  RollbackOutlined,
...
<template>
  <div class="payment-list">
    ...
    <a-card :bordered="false" class="table-card">
      <a-table :columns="columns" :data-source="payments" :loading="loading" :pagination="pagination" row-key="id"
        :scroll="{ x: 1100 }" @change="handleTableChange">
    ...
    <!-- Refund Modal -->
    <a-modal v-model:open="showRefundModal" title="Process Refund" @ok="handleRefund" :confirm-loading="refunding"
      ok-text="Process Refund" ok-type="primary">
      <div v-if="refundPayment" class="refund-form">
        <a-alert type="warning" message="This action cannot be undone" show-icon style="margin-bottom: 16px" />
        ...
        <a-form layout="vertical" style="margin-top: 16px">
          <a-form-item label="Refund Amount">
            <a-input-number v-model:value="refundAmount" :min="0" :max="refundPayment.amount" style="width: 100%">
              <template #addonBefore>৳</template>
            </a-input-number>
          </a-form-item>
          <a-form-item label="Reason for Refund">
            <a-textarea v-model:value="refundReason" placeholder="Enter reason for refund" :rows="3" />
          </a-form-item>
        </a-form>
      </div>
    </a-modal>
