      ...
      <div class="nav-content">
        <div class="nav-left">
          ...
          <a-breadcrumb class="nav-breadcrumb">
            <a-breadcrumb-item>
              <a-button class="flex items-center" size="small" type="link" @click.prevent="currentFolderId = null">
                <HomeOutlined />
                <span>All Files</span>
              </a-button>
            </a-breadcrumb-item>
            ...
            <a-breadcrumb-item v-for="treeItem in folderNameTree" :key="treeItem.id">
              <a-button size="small" type="link" @click.prevent="currentFolderId = treeItem.id">
                  ...
                  <div class="row-left">
                    <div class="file-thumb">
                      <a-image v-if="isImage(file.mime_type)" :src="getFileUrl(file.file_path)" class="thumb-image"
                        :preview="isSelectMode ? false : { src: getFileUrl(file.file_path) }" @click="selectFile(file)" />
                      <FileTextFilled v-else class="thumb-icon" />
                    </div>
                    <div class="row-details">
                      <div class="row-name">{{ file.original_name }}</div>
                      <div class="row-meta">
                        {{ formatSize(file?.size ?? 0) }} • {{ getFileExtension(file.original_name ?? '').toUpperCase()
                        }}
                      </div>
                    </div>
                  </div>
